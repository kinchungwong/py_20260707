...
Run
---
    python claude/spikes/piano_keyboard.py   # headless: render the keyboard to a PNG
"""
...
def _render_preview():
    """Headless: render the unpressed keyboard to a PNG so the widget can be
    eyeballed without a display."""
    import os
    ...
    render_full(screen, white_keys, black_keys, held=set())
    out = "claude/spikes/piano_keyboard_preview.png"
    pygame.image.save(screen, out)
    pygame.quit()
    print(f"{N_WHITE} white + {len(black_keys)} black keys, {WIDTH}x{HEIGHT}; wrote {out}")
